...
export const HELP_CONTENT: Record<string, HelpContent> = {
  ...
  map: {
    ...
    what: [
      ...
      "Above the map itself sit five sub-tabs: Map (the starmap you're reading about here), Gate Check, Likely Gate Camps, Local Threat, and D-Scan.",
    ],
    how: [
      ...
      "Hovering (without clicking) any system shows a transient tooltip with its security status, recent kill count, and - if it's had kills - a small clickable killboard of the most recent ones, each row jumping straight to that kill's full detail on the Killboard page. Clicking a system instead of just hovering pins that same tooltip open (with a close X) so you can move your mouse into it and actually click a kill row without it vanishing first; clicking the same system again, or clicking empty space, unpins it.",
      ...
      "Gate Check, Likely Gate Camps, Local Threat, and D-Scan are separate sub-tabs above the map itself, not overlays on top of it - switch to them the same way you'd switch any tab. Local Threat is a bulk affiliation/threat check: paste a Local chat member list (names separated by new lines or commas, since real character names often contain spaces) and each resolved pilot gets a card with portrait, corp/alliance, a Last Kill line (when and in which system they most recently killed someone, colored red within the last hour and orange within the last day so an actively-hunting pilot jumps out at a glance, plus how many gate-jumps that system is from your own current location if you've set one on the map), two more lines if you've set a current location - \"Last time they murdered someone in [your system]\" (when they most recently killed someone in that exact system specifically, or \"Never\" - a much stronger \"are they actually hunting where I am\" signal than the Last Kill line alone, since that one can point anywhere in New Eden) and \"Last time they themselves were killed in [your system]\" (the mirror of that, for their losses instead of their kills) - and a threat score derived from zKillboard's danger ratio - High needs both a very high danger ratio and at least 5 kills, Medium needs a high ratio or at least 20 kills, so one lucky kill can't outrank hundreds of real ones; anyone with zero combined kills/losses sorts to the bottom regardless of ratio, and each run is capped at 100 pilots. D-Scan takes a pasted Directional Scan result (Ctrl+A, Ctrl+C in the D-Scan window) and counts how many of each object type it found, using only the Type column of that tab-separated Name/Type/Distance format - any line that doesn't split into exactly those three fields is silently skipped.",
    ],
    gives: "A live read on where danger or opportunity is right now, an alert the moment a kill lands within whatever radius you've set of wherever you last clicked, a one-click path from \"something just died here\" straight to that kill's full detail, and - without leaving the page - a threat check on a room full of unfamiliar names or an instant breakdown of what's sitting on a directional scan.",
  },
  ...
  industry: {
    ...
    how: "Pick a blueprint or item, set your skills and any structure/rig/facility bonuses, and the relevant calculator does the material, time, cost, or odds math using live market prices and your real trained skill levels where relevant. Reprocessing has a \"Save as Default\" button that remembers your usual facility/skill setup; Production instead saves a per-system facility setup to System Presets, so a place you build often (your home system, a trade hub) can be picked from a list with its Trade Hub/structure/tax all restored exactly, independent of whichever blueprint you're calculating.",
  ...
  "industry.production": {
    title: "Production",
    what: "Full recursive build-cost calculation for anything with a manufacturing or reaction blueprint - the whole material tree, not just the top-level inputs, with a build-vs-buy comparison made at every single node using real Trade Hub prices, not a generic galaxy-wide index.",
    how: [
      "Search for the item or reaction, set Runs, Material/Time Efficiency, structure type (NPC Station vs. Engineering Complex, which applies real ME/TE bonuses on top), facility tax, and which Trade Hub to price everything against. Optionally pick a system to price in a real job cost off that system's live System Cost Index - leave it blank and the total is materials only. Structure Role Bonus % and Clone Type (Omega/Alpha) feed the same job cost formula the in-game Industry window uses, but both are manual entry for now - neither a structure's own role bonus nor a character's Alpha/Omega status has a public ESI field to read them from automatically. \"Save This System\" (next to the System field) saves the Trade Hub/structure/facility tax/role bonus/clone type setup against that system specifically, so picking it from System Presets later restores every one of those fields exactly - it deliberately leaves out the blueprint/Runs/ME/TE, since a saved system is a place you build, not a build, and shouldn't disturb whatever you're currently calculating.",
      "Calculate Build Cost expands into the Build Steps tree. Each row shows Qty (units needed at this stage), Build Cost (what building that quantity from its own materials would cost - blank if nothing can build it), Buy Cost (what buying that quantity outright costs), and Total (the cheaper of the two for a raw material, or the combined cost of just the materials one tier below for anything with its own recipe) - whichever of Build/Buy actually wins is highlighted. A small factory icon marks anything with its own blueprint or reaction formula even when Buy wins, so you know what's worth drilling into.",
      "Click a row to expand it - that automatically unticks the row and ticks its own materials, so the Selected Total below switches from \"this as one unit\" to \"its own ingredients\" the moment you look inside it; collapsing it puts it back. Collapse All resets every open tier at once. Tick or untick anything (already have some in a hangar? untick it) to build a subset, and Copy to Clipboard exports whatever's ticked as Name/Quantity lines, ready to paste straight into the game's own Multibuy window.",
    ],
    gives: "A real total cost priced off actual Trade Hub sell orders, a build-vs-buy verdict for every material in the chain, a Multibuy-ready shopping list for exactly the items you tick, and - when a system is picked - a full Job Cost breakdown (Estimated Item Value, System Cost Index, Structure Role Bonus, Facility Tax, SCC Surcharge, Alpha Clone Tax, Total Job Cost) laid out the same way the in-game Industry window's own job-cost panel shows it.",
  },
